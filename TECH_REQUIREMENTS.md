...
> **`public/search-index.json` is gitignored** — it is generated by the prebuild step (`scripts/build-search-index.ts`) and written directly to `public/` so `SearchBar` can fetch it via HTTP at runtime. It is NOT stored in `lib/generated/`. See TECH_REQUIREMENTS.md §22.1 and IMPLEMENTATION_PLAN.md Phase 12.
