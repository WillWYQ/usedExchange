...
| Catalog PDF export | Generate and download a single combined PDF catalog of all public-visible items (cover page, table of contents, per-category sections, one page per item with a live link) directly from Studio; an advisory readiness check flags items missing photos, a description, or a price before you generate |
