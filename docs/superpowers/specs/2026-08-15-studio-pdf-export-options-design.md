...
- **Backward compatibility, corrected during plan research:** the initial assumption that the `EN_FALLBACK` merge alone makes a `configDefaults.ts` entry unnecessary is only half the picture. `siteConfig.i18n.translations` is typed `Record<string, Partial<UIStrings>>` (`lib/config/types.ts:127`), so TypeScript never requires the new keys in a downstream site's config — safe by construction. But there is a second, independent gate: `pnpm build` runs `scripts/check-config.ts`, which fails the build if a locale in `availableLocales` is missing any key listed in `REQUIRED_UI_STRING_KEYS` (`scripts/lib/i18nRequiredKeys.ts`) from *both* its own dict and the default locale's dict — this check knows nothing about `EN_FALLBACK` (a separate module it doesn't import). `REQUIRED_UI_STRING_KEYS` is a curated allowlist, not every `UIStrings` key — e.g. `filterPriceBucketAll` (added post-v1.4.1, and already present in `configDefaults.ts`, `scripts/lib/configDefaults.ts:50-57`) is deliberately excluded from it. Two concrete rules follow: (1) the new PDF-chrome keys must **not** be added to `REQUIRED_UI_STRING_KEYS` — doing so would make `check-config` hard-fail for any downstream site with a second locale enabled whose config predates this feature; (2) per Rule 8 checklist step 4, and matching the `filterPriceBucketAll` precedent exactly, the new keys **do** get a `scripts/lib/configDefaults.ts` entry, so `pnpm update-site`/`pnpm migrate-config` auto-splice them into downstream `content/config.ts` files (this is about keeping the seller's own file complete and editable, not about avoiding a build failure — `EN_FALLBACK` already prevents that at runtime). They are also added to `content/config.ts`'s explicit `en` translations block and the commented-out `zh` template block directly in this repo, per Rule 8 checklist step 3.
...
`handleExportPdf` (`scripts/lib/studioApi.ts:1336`) follows the established `parseJsonBody(req.body, schema)` convention every other mutating route already uses (e.g. `bulkStatusBodySchema`, `scripts/lib/studioApi.ts:413-416` — an explicit `z.enum` is used deliberately, "the only way to reject bad input" per that call site's own comment, rather than a schema with `.catch()` that would silently coerce a typo into a valid value). This feature follows the same reject-don't-coerce rule rather than substituting silent defaults for bad input:
...
- `priceStrategy` and each element of `statuses`: rejected with `400` by the zod enum itself if not one of the fixed values — no fallback substitution. `statuses` may be an empty array (a seller unchecking every box is a legitimate, explicit choice — see §3.1); `min(1)` is deliberately *not* applied here, unlike `bulkStatusBodySchema`'s `ids`.
- `locale`: not a static enum (the valid set is the seller's own `siteConfig.i18n.availableLocales`, unknown at schema-definition time) — checked in the handler body with an explicit `if (!siteConfig.i18n.availableLocales.includes(locale)) throw new StudioError(400, ...)`, matching the `throw new StudioError(400, ...)` pattern used for other request-shape checks elsewhere in this file (e.g. `applyStatus`, `scripts/lib/studioApi.ts:446`).
- `categories`: passed through unvalidated — `groupEligibleItems` only ever emits groups for categories that exist in `loadCategories()`, so an unknown/stale slug in the filter set is already a harmless no-op, and rejecting it would only make the endpoint more fragile against category renames with no safety benefit.
