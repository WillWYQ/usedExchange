...
- `listCategorySummaries(projectRoot): Promise<CategorySummary[]>` — reads every directory under `content/items/` that passes `isValidSlug` (naturally excluding `_defaults.json`, `_template.json`, and any `_`-prefixed folder), reads each one's metadata, and counts items via a new `countCategoryItems(categoryDir)` (a direct read of subfolders containing `item.json`). This deliberately does **not** call `loadAllItemsRaw()` (the function `listStudioItems` uses for the item table): that loader resolves `content/` from `process.cwd()`, not from the `projectRoot` parameter threaded through every other function in this module — harmless in production (`pnpm studio` always runs from the repo root, so the two are equal) but it would silently ignore a sandboxed `projectRoot` in tests. `countCategoryItems` reads the given directory directly instead, so it is correct under test sandboxes regardless of that pre-existing quirk.
...
- `GET /api/categories`: empty `content/items/` → `[]`; a folder with no `_category.json` → all-default metadata + correct `itemCount`; a folder with a sparse `_category.json` → only the set fields differ from default; `itemCount` matches the number of subfolders containing `item.json`, and ignores a stray subfolder that doesn't.
