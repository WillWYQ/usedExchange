...
Studio 是单个卖家每周会打开好几次的工具——密集表格加一个抽屉表单。没有 hero。密度与可读性优先，大胆只花在一处。

### 14.1 主体词汇表

二手转卖的物质世界是寄售单据：复写纸（NCR）三联单、手写价签、热敏快递标签、橡皮图章。项目本身已有 shipping calculator 与 `sold_date` 字段，所以这套词汇是字面意义上的，不是借来的隐喻。

### 14.2 色彩 —— 复写纸联单，不是奶油色纸

| 令牌 | 值 | 用途 |
|---|---|---|
| `carbon-pale` | `#E4EBEF` | 页面底色 —— 复写纸第二联的冷灰蓝 |
| `carbon-rule` | `#C2CFD6` | 表格分隔线、字段下划线 |
| `ink` | `#1B2A35` | 正文与数据 —— 钢笔墨的冷黑 |
| `ink-soft` | `#5C6E7A` | 次要标签、说明文字 |
| `stamp` | `#B3241E` | **仅用于 `sold`** —— 图章红 |
| `pending` | `#8A6A12` | `pending` 状态 —— 档案黄褐 |

「奶油底 + 展示衬线 + 陶土色」被主动否决：那是当前 AI 默认三种观感之一，且不论主体是什么都会出现。复写纸的冷灰蓝有同样的纸感，但它属于单据而非精品店；冷底色让图章红真正承载信息，暖底色会把它压平。

### 14.3 字体

全部通过 `@fontsource` npm 包本地安装，绝不走 CDN —— 本地工具必须离线可用。

- **数据 / 工具字：** `Courier Prime`。价格、日期、尺寸、文件名。等宽让金额与日期天然对齐，也是复写纸这一参照真正落地的地方。
- **界面字：** `IBM Plex Sans`。中性，小字号清晰。
- **标题 / 状态标签：** `Archivo Narrow`。窄体，来自标签与表单栏头 —— 不是展示衬线。
...
结构装置只编码真实信息。左栏数字是真实计数；不做 01 / 02 / 03 编号，因为商品列表不是序列。顶栏把「未提交变更数」放在最显眼处，因为改了不推送是这个工具唯一真正的失败模式。

### 14.5 签名元素：SOLD 橡皮图章

批量标记售出时，图章印记压在选中行上 —— 轻微旋转（约 -4°）、墨迹边缘不匀、一次性压印动画，随后该行沉降为已售态。这是唯一的大胆元素；周围一切保持安静：无装饰性圆角、无渐变、无堆叠阴影。

它配得上签名位，因为它**就是**这个工具存在的理由：「这周末卖掉三件，一次全标完。」

在 `prefers-reduced-motion` 下直接渲染终态，不做压印动画。

### 14.6 质量底线

响应式到窄屏（表格折叠为卡片）、键盘焦点可见、尊重 reduced motion。在此声明一次，不在界面里张扬。
