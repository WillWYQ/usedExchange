...
it("keeps TEMPLATE_PATHS and both UPDATE_GUIDE path lists in sync", async () => {
  const source = await fs.readFile("scripts/update-site.ts", "utf-8");
  const en = await fs.readFile("docs/UPDATE_GUIDE.md", "utf-8");
  const zh = await fs.readFile("docs/UPDATE_GUIDE_zh.md", "utf-8");
  // The comment above TEMPLATE_PATHS says "keep in sync with Step 2 of
  // UPDATE_GUIDE". This is that instruction, enforced.
  for (const p of ["scripts", "studio", "docs", "lib", "app"]) {
    expect(source).toContain(`"${p}"`);
    expect(en).toMatch(new RegExp(`\\b${p}\\b`));
    expect(zh).toMatch(new RegExp(`\\b${p}\\b`));
  }
});
```
