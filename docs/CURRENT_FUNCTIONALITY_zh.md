...
2. 在 content/config.ts 中添加 translations.{locale} 块，翻译全部 67 个 UI 字符串键
...
- **对卖家：** 添加新语言需要两步：
  1. 将语区代码加入 `siteConfig.i18n.availableLocales`（如 `["en", "zh"]`），**并**在 `content/config.ts` 中添加包含全部 67 个 UI 字符串键（已翻译）的 `translations.{locale}` 块。该块缺失或不完整时构建将失败。
...
| 国际化 | `i18n.defaultLocale`、`i18n.availableLocales`、`i18n.showLocaleSwitcher`、`i18n.translations.{locale}.*`（67 个 UI 字符串键） |
