...
| `POST /api/import-url/preview` | SSRF-safe fetch (`scripts/lib/ssrfGuard.ts`) of a seller-supplied `{ url }`, then extracts (`scripts/lib/urlImport.ts`) a best-guess `name` and a filtered, deduped list of candidate photo `images` (absolute URLs) — nothing is downloaded or written yet. If the fast fetch finds zero images, retries once via an SSRF-guarded headless-Chromium render (`scripts/lib/headlessImport.ts`) before giving up. Response gains `usedHeadlessFallback: boolean` and `headlessFailureReason: "not-installed" \| "navigation-failed" \| null`. `400` with `{ error }` for a rejected/failed fetch (bad scheme, disallowed address, timeout, oversized body). A non-HTML response returns `{ name: null, images: [], usedHeadlessFallback: false, headlessFailureReason: null }` rather than parsing binary bytes as text. |
| `POST .../images/import` | Body `{ urls: string[], sourceUrl?: string }` (max 24 URLs) — downloads each URL through the same SSRF-safe fetch, sniffs the real image type from bytes (never the URL extension or remote `Content-Type`), and writes it via the existing `writeImage`/collision-avoiding pipeline. When `sourceUrl` is provided, it is sent as an origin-only `Referer` header on each download. Per-URL failures never fail the batch: response is always `200` with `{ files, imported, failed: [{ url, error }] }`. |
...
| `POST /api/export-pdf` | Starts a combined catalog PDF render and streams progress back as **SSE** — always `200`, and the PDF's bytes never ride this response. Body: `{ locale, priceStrategy, categories, statuses }` — `priceStrategy` is one of `lowest`/`highest`/`pickup`/`shipping`/`average`; `statuses` any subset of the 5 `Status` values; `categories` any list of category slugs. `400` with `{ error }` only for a request-body validation failure (bad `priceStrategy`/`status`/unlisted `locale`) — that check runs before the stream opens. Once streaming, each frame is `event: progress` with a `PdfExportProgress` payload (`{ stage }`, plus `completed`/`total` during the two `images-pass-*` stages), ending in either `event: done` with `{ token }` or `event: error` with `{ error }` (no eligible items after filtering, or a Chromium render failure both surface this way, not as an HTTP error status). Redeem `token` via the download route below to get the actual file. |
| `GET /api/export-pdf/download/<token>` | Redeems a `done` event's `token` for the rendered PDF (`application/pdf`). Released — map entry and temp file both deleted — once that download's bytes are confirmed sent, or after 5 minutes unredeemed, whichever comes first. `404` with `{ error }` for an unknown, already-redeemed, or expired token. |
