...
> ℹ️ **如果你希望物品详情页的"下载 Flyer"按钮能包含照片，这一步是必需的。** 上传通过 S3 SDK 在你的机器上以服务端方式完成，站点其他地方的照片都通过普通 `<img>` 标签渲染——两者都不需要浏览器↔存储桶的 CORS。Flyer 按钮是唯一的例外：它需要在访客的浏览器里获取每张照片的原始字节，嵌入生成的 PDF，而浏览器在没有 CORS 策略时会阻止跨域获取。跳过这一步 flyer 仍然能正常生成，只是不会带照片。

最简单的方式——在你的机器上运行一次（会从 `.env.local` 读取 `CF_R2_*`，从 `content/config.ts` 读取站点的 `baseUrl`；只会新增一条规则，不会动存储桶上已有的其他 CORS 规则）：

```bash
pnpm configure-image-cors
```

如果你的 R2 API Token 没有存储桶设置权限，命令会失败并打印出手动配置的策略。手动配置的方式：你的存储桶 → **Settings** → **CORS Policy** → 添加：
